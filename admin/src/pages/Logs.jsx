...
export default function Logs() {
  ...
  async function cargar() {
    setLoading(true)
    const { data } = await supabase.from('mensajes_log').select('*').order('created_at', { ascending: false }).limit(200)
    setLogs(data || [])
    setLoading(false)
  }
  ...
  return (
    <>
      <div className="page-header">
        ...
        <button className="btn btn-outline" onClick={cargar}><RefreshCw size={13} /> Actualizar</button>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input className="form-input" style={{ width: 240, paddingLeft: 32 }} placeholder="Buscar número o mensaje..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
          </div>
        </div>
        {loading ? (
          ...
          <table className="table">
            <thead><tr><th>Fecha</th><th>Contacto</th><th>Mensaje recibido</th><th>Respuesta del bot</th></tr></thead>
            <tbody>
              {filtrados.map(log => (
                <tr key={log.id}>
                  <td style={{ whiteSpace: 'nowrap', fontSize: 12, color: 'var(--text-3)' }}>{fmt(log.created_at)}</td>
                  <td>
                    <div style={{ fontWeight: 500, fontSize: 13 }}>{log.nombre || '—'}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{log.numero?.replace('@c.us', '')}</div>
                  </td>
                  <td style={{ maxWidth: 200, fontSize: 13 }}><div style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{log.mensaje}</div></td>
                  <td style={{ maxWidth: 260, fontSize: 12, color: 'var(--text-2)' }}>
                    <div style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                      {log.respuesta ? log.respuesta.slice(0, 100) + (log.respuesta.length > 100 ? '…' : '') : '—'}
                    </div>
                  </td>
                </tr>
              ))}
